...
export default function QuizPlayPage({ params }: PageProps) {
  ...
  useEffect(() => {
    const loadQuestions = async () => {
      ...
      try {
        setError(null);
        // Construct the correct path for the chapter
        const chapterPath = quiz.chapter.includes("/") 
          ? quiz.chapter 
          : `part-${quiz.part}/${quiz.chapter}`;
        
        const response = await fetch(`/data/questions/${chapterPath}.json`);
        if (!response.ok) {
          throw new Error(`Failed to load questions. Please try again later.`);
        }
        const data = await response.json();
        // Handle both formats: { questions: [...] } and [...]
        const allQuestions: Question[] = Array.isArray(data) ? data : data.questions;
